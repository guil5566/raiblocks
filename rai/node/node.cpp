...
void rai::election::broadcast_winner (rai::transaction & transaction)
{
...
void rai::active_transactions::announce_votes ()
{
	...
	for (auto i (roots.begin ()), n (roots.end ()); i != n; ++i)
	{
		...
		else
		{
			...
			auto announcements (i->announcements);
			node.background ([election_l, announcements]() {
				rai::transaction transaction (election_l->node.store.environment, nullptr, false);
				if (announcements > 0)
				{
